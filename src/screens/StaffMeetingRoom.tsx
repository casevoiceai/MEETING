...
export default function StaffMeetingRoom() {
  ...
  async function routeWithJulie(message: string): Promise<string[]> {
    ...
    try {
      ...
      if (parsed.line) {
        addMessage({ speaker: "Julie", text: parsed.line, isJulie: true });
      }
  ...
  async function handleSend() {
    ...
    try {
      ...
      for (const mentor of mentors.slice(0, 2)) {
        ...
        if (response) {
          addMessage({ speaker: mentor, text: response });
        }
      }
    } catch {
  ...
  async function handleSaveSession() {
    ...
    const title = `Founder CRM Session -- ${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`;

    try {
      const result = await saveMeetingToDrive();
      if (result?.success) {
        setSaveStatus("Session saved to Google Drive.");
      } else {
        setSaveStatus("Save failed. Check Google Drive connection.");
      }
    } catch {
    ...
    <div className="flex flex-col h-full" style={{ backgroundColor: "#08111F" }}>
      <div className="flex items-center gap-3 px-5 py-3 border-b flex-shrink-0" style={{ borderColor: BORDER }}>
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          >
            {callingMember
              ? <><RefreshCw size={10} className="animate-spin" /> {callingMember}...</>
              : <><Users size={10} /> Call Team</>
            }
          </button>
          <button
      ...
      {showCallPanel && (
        <div className="px-5 py-3 border-b" style={{ borderColor: BORDER, backgroundColor: NAVY }}>
          <p className="text-[10px] mb-2 font-bold tracking-widest uppercase" style={{ color: DIM }}>
      ...
      <div className="px-5 py-4 border-t flex-shrink-0" style={{ borderColor: BORDER }}>
        ...
        <p className="text-[9px] mt-1.5" style={{ color: DIM }}>
