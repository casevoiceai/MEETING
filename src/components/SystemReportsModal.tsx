...
export default function SystemReportsModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return createPortal(
    <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-[9500]" onClick={onClose}>
      <div 
        className="bg-[#0D1B2E] border border-[#1B2A4A] w-[650px] h-[520px] flex flex-col rounded-xl shadow-2xl"
        onClick={e => e.stopPropagation()}
      >
        <div className="p-4 border-b border-[#1B2A4A] flex justify-between items-center bg-[#111D30]">
          <span className="text-white font-bold text-xs tracking-tighter">SYSTEM REPORTS</span>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {reports.length === 0 ? (
            ...
            reports.map((r: any) => (
              <div key={r.id} className="p-3 bg-[#16253A] border border-[#1B2A4A] rounded flex flex-col gap-2">
                ...
                <div className="text-[9px] text-[#C9A84C] font-mono">
                  TYPE: {r.type}
                </div>
              </div>
            ))
